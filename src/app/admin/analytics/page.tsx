...
import { BarChart3, Users, Eye, Globe, Monitor, Smartphone, ArrowUpRight, RefreshCw } from 'lucide-react';
      ...
      {data && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            <SummaryCard icon={<Eye className="w-5 h-5 text-blue-600" />} label="Page Views" value={data.summary.totalPageviews} bg="bg-blue-50" />
            <SummaryCard icon={<Users className="w-5 h-5 text-green-600" />} label="Unique Visitors" value={data.summary.uniqueVisitors} bg="bg-green-50" />
            <SummaryCard icon={<BarChart3 className="w-5 h-5 text-purple-600" />} label="Pages / Visitor" value={data.summary.uniqueVisitors > 0 ? (data.summary.totalPageviews / data.summary.uniqueVisitors).toFixed(1) : '0'} bg="bg-purple-50" />
            <SummaryCard icon={<Globe className="w-5 h-5 text-orange-600" />} label="Countries" value={data.topCountries.length} bg="bg-orange-50" />
          </div>
              ...
              <div className="flex items-end gap-1 h-48">
                {data.dailyTrend.map((day) => (
                  <div key={day.date} className="flex-1 flex flex-col items-center group relative">
                    ...
                    <div className="w-full bg-green-500 rounded-t hover:bg-green-600 transition min-h-[2px]" style={{ height: `${(day.views / maxViews) * 100}%` }} />
                    <span className="text-[10px] text-gray-400 mt-1 rotate-[-45deg] origin-top-left">{day.date.slice(5)}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            <div className="bg-white rounded-xl shadow-sm border p-6">
              ...
              <div className="space-y-3">
                {data.topPages.length === 0 && <p className="text-gray-400 text-sm">No pages tracked yet</p>}
                {data.topPages.map((page) => (
                  <div key={page.path} className="flex items-center justify-between">
                    <a href={`https://getfencefind.com${page.path}`} target="_blank" rel="noopener noreferrer" className="text-sm text-gray-700 hover:text-green-600 transition truncate max-w-[60%] flex items-center gap-1">
                      {page.path}
            ...
            <div className="bg-white rounded-xl shadow-sm border p-6">
              ...
              <div className="space-y-3">
                {data.topReferrers.length === 0 && <p className="text-gray-400 text-sm">No referrer data yet</p>}
                {data.topReferrers.map((ref) => (
                  <div key={ref.referrer} className="flex items-center justify-between">
                    <span className="text-sm text-gray-700 truncate max-w-[70%]">{ref.referrer}</span>
